...
define('TEMP_DIR', '/tmp/schema_deploy');
define('LOG_FILE', '/var/log/schema_deploy.log');
define('STATE_FILE', '/var/lib/schema_deploy/state.json');
...
function deploy_zip($domain, $zipUrl, $user, $redirectsData) {
    ...
    if (!$success || !file_exists($zipFile) || $httpCode != 200) {
        ...
        if (!empty($redirectsData['redirects']) || isset($redirectsData['mirror'])) {
            $redirectsJson = json_encode($redirectsData, JSON_PRETTY_PRINT);
            file_put_contents("$webRoot/redirects.json", $redirectsJson);
            log_message("Created redirects.json for $domain despite ZIP download failure");
        }
    ...
    if (!empty($redirectsData['redirects']) || isset($redirectsData['mirror'])) {
        $redirectsJson = json_encode($redirectsData, JSON_PRETTY_PRINT);
        file_put_contents("$webRoot/redirects.json", $redirectsJson);
        log_message("Created redirects.json for $domain with " . count($redirectsData['redirects']) . " redirects" .
