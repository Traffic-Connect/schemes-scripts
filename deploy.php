...
function create_domain($domain, $user) {
    ...
    if ($returnVar === 0 && !empty($output)) {
        ...
        if (!empty($existingUser) && $existingUser !== $user) {
            log_message("Domain belongs to another user: $domain, deleting");
...
function update_redirects($hestiaDomain, $schemaUser, $redirectsData) {
    ...
    if (redirects_need_update($webRoot, $redirectsData)) {
        ...
        if ($result === false) {
            log_message("Failed to write redirects for $hestiaDomain");
            return false;
        }
...
function deploy_zip($domain, $zipUrl, $user, $redirectsData) {
    ...
    log_message("Deployed ZIP: $domain");
}
...
function main() {
    log_message("Starting schema deployment");
    ...
    log_message("Schema deployment completed");
}
